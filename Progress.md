...
- `ruby scripts/validate_notes_index.rb` 已通过：`notes index ok: 144 entries, 144 top-level note html files`。下一步继续执行 `git diff --check`、Jekyll 隔离构建、四篇个案与系列页的桌面/手机浏览器回归，完成后只提交本批改动并 push。
